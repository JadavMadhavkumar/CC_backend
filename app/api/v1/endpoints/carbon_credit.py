...
from app.schemas.carbon_credit import CarbonCreditResponse, CarbonCreditStats, CarbonCreditRetire
...
async def retire_carbon_credit(
    ...
    credit = await CarbonCreditService.retire_credit(db, credit_id, retire_data.reason)
...
async def get_carbon_credit_stats(
    ...
    credits = await CarbonCreditService.get_credits_by_organization(
        db, organization_id=org_id, skip=0, limit=10000
    )
    ...
    verified_credits = sum(
        c.quantity for c in credits if c.verification_level in ["verified", "gold"]
    )
    ...
    for credit in credits:
        credits_by_category[credit.category] = (
            credits_by_category.get(credit.category, 0) + credit.quantity
        )
