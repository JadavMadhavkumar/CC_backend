...
class CarbonCredit(Base):
    ...
    sub_category: Mapped[str | None] = mapped_column(String(100), nullable=True, index=True)
    vintage_year: Mapped[int] = mapped_column(nullable=False, index=True)
    ...
    unit: Mapped[str] = mapped_column(String(50), nullable=False, default="tCO2e")
    ...
    emissions_removed: Mapped[float] = mapped_column(Numeric(20, 6), default=0, nullable=False)
    ...
    third_party_verifier: Mapped[str | None] = mapped_column(String(255), nullable=True)
    registry_name: Mapped[str | None] = mapped_column(String(100), nullable=True)
    registry_id: Mapped[str | None] = mapped_column(String(100), nullable=True)
    blockchain_tx_hash: Mapped[str | None] = mapped_column(String(200), nullable=True, index=True)
    price_per_credit: Mapped[float | None] = mapped_column(Numeric(20, 2), nullable=True)
    ...
    credit_metadata: Mapped[str | None] = mapped_column("metadata", Text, nullable=True)
    ...
    organization: Mapped["Organization"] = relationship(
        "Organization", back_populates="carbon_credits", foreign_keys=[organization_id]
    )

    transactions: Mapped[list["CarbonCreditTransaction"]] = relationship(
        "CarbonCreditTransaction", back_populates="carbon_credit", cascade="all, delete-orphan"
    )

    def __repr__(self) -> str:
        return (
            f"<CarbonCredit(id={self.id}, serial_number={self.serial_number}"
...
class CarbonCreditTransaction(Base):
    ...
    carbon_credit_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, ForeignKey("carbon_credits.id"), nullable=False, index=True
    )
    transaction_type: Mapped[str] = mapped_column(String(50), nullable=False)
    from_organization_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, nullable=True)
    to_organization_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, nullable=True)
    quantity: Mapped[float] = mapped_column(Numeric(20, 6), nullable=False)
    ...
    transaction_metadata: Mapped[str | None] = mapped_column("metadata", Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=datetime.utcnow
    )

    carbon_credit: Mapped["CarbonCredit"] = relationship(
        "CarbonCredit", back_populates="transactions"
    )

    def __repr__(self) -> str:
        return (
            f"<CarbonCreditTransaction(id={self.id}, type={self.transaction_type}"
            f", quantity={self.quantity})>"
        )
