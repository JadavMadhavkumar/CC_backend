...
class VerificationRequest(Base):
    ...
    assigned_to_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, nullable=True, index=True)
    ...
    methodology_used: Mapped[str | None] = mapped_column(String(500), nullable=True)
    verification_standard: Mapped[str | None] = mapped_column(String(100), nullable=True)
    compliance_score: Mapped[float | None] = mapped_column(Numeric(10, 2), nullable=True)
    data_accuracy_score: Mapped[float | None] = mapped_column(Numeric(10, 2), nullable=True)
    completeness_score: Mapped[float | None] = mapped_column(Numeric(10, 2), nullable=True)
    consistency_score: Mapped[float | None] = mapped_column(Numeric(10, 2), nullable=True)
    transparency_score: Mapped[float | None] = mapped_column(Numeric(10, 2), nullable=True)
    findings: Mapped[str | None] = mapped_column(Text, nullable=True)
    issues: Mapped[str | None] = mapped_column(Text, nullable=True)
    recommendations: Mapped[str | None] = mapped_column(Text, nullable=True)
    conclusion: Mapped[str | None] = mapped_column(Text, nullable=True)
    approved_credits: Mapped[float] = mapped_column(Numeric(20, 6), default=0, nullable=False)
    rejected_credits: Mapped[float] = mapped_column(Numeric(20, 6), default=0, nullable=False)
    request_metadata: Mapped[str | None] = mapped_column("metadata", Text, nullable=True)
    ...
    organization: Mapped["Organization"] = relationship(
        "Organization", back_populates="verification_requests", foreign_keys=[organization_id]
    )

    def __repr__(self) -> str:
        return (
            f"<VerificationRequest(id={self.id}, entity_type={self.entity_type}"
...
class AuditLog(Base):
    ...
    processing_time_ms: Mapped[float | None] = mapped_column(Numeric(20, 2), nullable=True)
