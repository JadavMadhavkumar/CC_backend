...
class WasteRecord(Base):
    ...
    plastic_type: Mapped[str | None] = mapped_column(
    ...
    calorific_value: Mapped[float | None] = mapped_column(
        Numeric(20, 4), nullable=True
    )
    processing_method: Mapped[str] = mapped_column(
        String(100), nullable=False
    )
    processing_facility: Mapped[str | None] = mapped_column(
    ...
    emission_factor_used: Mapped[uuid.UUID | None] = mapped_column(
        Uuid, nullable=True
    )
    ...
    def __repr__(self) -> str:
        return f"<WasteRecord(id={self.id}, waste_type={self.waste_type}, quantity={self.quantity})>"